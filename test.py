...
from utils import *
from loss import OriTripletLoss, HcTripletLoss, CrossEntropyLabelSmooth
...
parser.add_argument('--hcloss', default='no', type=str, help='OriTri, HcTri')
parser.add_argument('--margin_hc', default=0, type=float,
                    metavar='margin', help='hc triplet loss margin')
...
def extract_gall_feat(gall_loader):
    ...
    start = time.time()
    ptr = 0
    gall_feat_pool = np.zeros((ngall, feature_dim))
    gall_feat_fc = np.zeros((ngall, feature_dim_att))
    with torch.no_grad():
...
def extract_query_feat(query_loader):
    ...
    start = time.time()
    ptr = 0
    query_feat_pool = np.zeros((nquery, feature_dim))
    query_feat_fc = np.zeros((nquery, feature_dim_att))
    with torch.no_grad():
...
if dataset == 'sysu':
    ...
    model_path = checkpoint_path + 'sysu_bpool_avg_cpool_max_hcloss_HcTri_fuse_sum_hcmargin_0.5_gm_ls_off_s1_drop_0.2_8_6_lr_0.1_seed_0_best.t'
    ...
    for trial in range(10):
        ...
        print(
            'Fuse:   Rank-1: {:.2%} | Rank-5: {:.2%} | Rank-10: {:.2%}| Rank-20: {:.2%}| mAP: {:.2%}| mINP: {:.2%}'.format(
    ...
    for trial in range(10):
        test_trial = trial +1
        model_path = checkpoint_path + 'regdb_bpool_max_cpool_max_hcloss_HcTri_fuse_sum_hcmargin_0_gm_ls_off_s1_drop_0.2_4_8_lr_0.1_seed_0_trial_{}_best.t'.format(test_trial)
        if os.path.isfile(model_path):
            print('==> loading checkpoint!')
            checkpoint = torch.load(model_path)
            net.load_state_dict(checkpoint['net'])
